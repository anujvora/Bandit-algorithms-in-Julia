...
########################################################
  ...
  # @show best_policy.u
        # prob = OptimizationProblem(compute_value_policy_TS_decay_greedy_knownT, x_0, [ num_arms, prob_opt_curr,  all_arms_list, means_of_arms_dist,sqrt(curr_time/hori)], lb = zeros(num_arms), ub = ones(num_arms) ,lcons = 1, ucons = 1)
        # best_policy = solve(prob,NelderMead()) 
        
        # lin_const = scipy.optimize.LinearConstraint(ones(num_arms), lb=1, ub=1)
        # optimizer_soln = scipy.optimize.minimize(compute_value_policy_TS_decay_greedy_knownT, x_0,  bounds=bounds_for_opt, constraints=lin_const, args=(num_arms, prob_opt_curr,  all_arms_list, means_of_arms_dist,sqrt(curr_time/hori)))
        # best_policy =  optimizer_soln.x  
        ...
        # @show arm_chosen
#######################################################
# function  gradient_compute_value_policy!(G,policy, params) 
...
#     # @show (1-coeff)*( sum( (sum((la.I- policy_in_mat).^2,dims = 1))*prior_prob ))/2
#     # @show (1-coeff)*( sum((sum((la.I- policy_in_mat).^2,dims = 1)).* prior_prob))/2,  (coeff)*sum(means_for_trans.* policy)
#     G = (  (1-coeff).*( prior_prob - policy)  -  (coeff).*means_for_trans )
                    
#     # @show policy, value_policy
#     # return gradient
# end

# function  hessian_compute_value_policy!(H,policy, params) 
